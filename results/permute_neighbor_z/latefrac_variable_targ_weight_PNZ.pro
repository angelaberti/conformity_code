...
PRO latefrac_variable_targ_weight_PNZ, outputFormat;, zmin, zmax;, dz_coeff, printEvery

	string_dm = '1.0'

	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/latefracplot_targ_weight_1.0dex_zall_PNZ', THICK=5, /ENCAP
	...
	PLOT, Rproj_array, float(neigh_late_grand_total)/neigh_all_grand_total, xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle='Projected Radius (Mpc)', ytitle='Late-type Fraction', $
	  title = 'M13 Mass Limit + 1.0 (0.0) for SF (Q) IPs ' + textoidl('(\Deltaz=2.0)'), /NODATA
	...
	SET_PLOT, 'X'
        ...
        mwrfits, outputStruct, '~/results/permute_neighbor_z/latefrac_' + strtrim(strcompress(string(zmin, format='(f20.1)')), 1) + '_' $
