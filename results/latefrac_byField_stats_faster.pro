...
PRO latefrac_byField_stats_faster, outputFormat
	input_dm = 1.0
...
;	ENDELSE
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/latefrac_byField_norm_diff_' + string_dm + 'dex', THICK=5, /ENCAP
...
;	!P.CHARSIZE = 1.25
	...
	PLOT, findgen(10), findgen(10), xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle='Projected Radius (Mpc)', ytitle='Normalized % diff. in late-type frac between LT/ET IPs', /NODATA, $
		title = 'Norm. conformity signal strength by field ' + textoidl('(M13+1.0dex; \Deltaz=2.0)')
...
;	PRINT, 'Length of masterArray: ', n_elements(masterArray)
	...
	RESTORE, 'latefrac_byField_data.sav'
...
;		fieldArray = [strtrim(fields[i],1), (fracIPSF - fracIPQ)/((fracIPSF + fracIPQ)/2.)]
		...
		meanArray = []
		ydata=masterArray[*,c]
		FOR x=0,6 DO meanArray = [meanArray,mean(ydata[8*x+0:8*x+7])]
		PRINT, meanArray

		OPLOT, findgen(8)/1., meanArray, LINESTYLE=0, color=cgColor(colors[i])
