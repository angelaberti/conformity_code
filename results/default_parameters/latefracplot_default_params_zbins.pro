PRO latefracplot_default_params_zbins, outputFormat
  IF (string(outputFormat) eq 'ps') THEN BEGIN
	PS_OPEN, '../../figures/latefracplot_default_params_zbins', THICK=5, /ENCAP
...
;		IF z_index ge nx*(ny-1) THEN title=textoidl('Default Sample Parameters') ELSE title=''
		IF z_index eq 3 THEN BEGIN
			XYOUTS, 4.25, 1.025, "Default Sample Parameters", ALIGNMENT=0.5 
		ENDIF
...
;			plot, data.Rmax, frac_IPSF, xrange=[xmin,xmax], yrange=[ymin,ymax], title=textoidl('Default Mass Limit; \Deltaz < 0.005(1+z)'), $
;			  xtitle='Projected Radius (Mpc)', ytitle='Late-type Fraction', thick=linethickness
			...
			IF z_index eq nx*ny-1 THEN BEGIN
				LEGEND, ["Late-type IP","Early-type IP"], linestyle=[0,2], box=0, /TOP, /RIGHT, charsize=xyoutscharsize
			ENDIF
...
;			sigmaRange075_300 = sigmaRange(inputFile,3,11)
			sigmaRange075_400 = sigmaRange(inputFile,3,15)
                        ...
                        xyouts, xmin+0.05*xr, ymin+0.05*yr, strtrim(string(zArray[z_index],format='(f20.1)'),1) + textoidl(" < z < ") + $
                                                            strtrim(string(zArray[z_index]+0.2,format='(f20.1)'),1), ALIGNMENT=0.0, charsize=xyoutscharsize
