...
PRO latefrac_BSE_IPmatchFBF_PHI37_allz_halo12, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	PHI = 3.7
...
;	Rproj_array = float(dRproj)*(findgen(n_annuli+1))	
	Rproj_array = [0.0, 0.5, 1.0+FINDGEN(15)]	
	Rplot_array = [0.25, 0.75, 1.5+FINDGEN(14)]	
...
;	zsuffix = 'T' + strtrim(i+1,1)
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/latefracplot_BSE_IPmatchFBF_PHI3.7_allz', THICK=3, /ENCAP
...
;	dataAll = dataAllallz[where( (dataAllallz.zprimus GE z_low) AND (dataAllallz.zprimus LE z_high) )]
	...
	PLOT, Rplot_array, float(neigh_late_grand_total)/neigh_all_grand_total, xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle='Projected Radius (Mpc)', ytitle='Late-type Fraction', $
