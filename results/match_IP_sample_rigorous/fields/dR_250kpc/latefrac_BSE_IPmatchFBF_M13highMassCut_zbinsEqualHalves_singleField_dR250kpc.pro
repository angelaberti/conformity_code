...
; M13highMassCut: IP matching procedure uses high mass cutoff threshold based on Moustakas13 stellar mass function results
; allz:		late-type fractions are computed for 0.2 < z < 1.0 

PRO latefrac_BSE_IPmatchFBF_M13highMassCut_zbinsEqualHalves_singleField_dR250kpc, outputFormat, field;, zmin, zmax;, dz_coeff, printEvery
	dz_coeff = 2.0
	zmin = 0.2
	zmax = 1.0

	; read in data files
        dataIPallz  = MRDFITS('~/conformity/results/match_IP_sample_rigorous/matchedIPsampleFBF_M13highMassCut.fits', 1)
	...
	; eliminate data with targ_weight < 1
        dataAllallz = dataAllallz[where((dataAllallz.targ_weight GE 1.) AND (strtrim(dataAllallz.field, 2) EQ field))]
        dataIPallz  = dataIPallz[where((dataIPallz.targ_weight GE 1.) AND (strtrim(dataIPallz.field, 2) EQ field))]

	; divide IP population into 2 redshift bins with equal numbers of IPs
...
;	dataIP  = dataIP[where(dataIP.targ_weight GE 1.)]
  ...
  FOR i=0,1 DO BEGIN
	...
	zsuffix = 'H' + strtrim(i+1,1) + '_' + field
	zlabel  = strtrim(string(z_low,format='(f20.2)'),1) + ' < z < ' + strtrim(string(z_high,format='(f20.2)'),1)

	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/latefracplot_BSE_IPmatchFBF_M13highMassCut_z' + zsuffix, THICK=3, /ENCAP
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/fields/dR_250kpc/latefrac_' + zsuffix + '_targ_weight_IPmatchFBF_M13highMassCut' + string_dR + '_BSE.fits', /create
  ENDFOR
END
