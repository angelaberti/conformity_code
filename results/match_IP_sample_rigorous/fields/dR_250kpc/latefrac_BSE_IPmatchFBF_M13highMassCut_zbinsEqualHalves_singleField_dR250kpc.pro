...
PRO latefrac_BSE_IPmatchFBF_M13highMassCut_zbinsEqualHalves_singleField_dR250kpc, outputFormat, field;, zmin, zmax;, dz_coeff, printEvery
	...
	; read in data files
        dataIPallz  = MRDFITS('~/results/match_IP_sample_rigorous/matchedIPsampleFBF_M13highMassCut.fits', 1)
...
;	dataIP  = dataIP[where(dataIP.targ_weight GE 1.)]
  ...
  FOR i=0,1 DO BEGIN
	...
	MWRFITS, outputStruct, '~/results/match_IP_sample_rigorous/fields/dR_250kpc/latefrac_' + zsuffix + '_targ_weight_IPmatchFBF_M13highMassCut' + string_dR + '_BSE.fits', /create
  ENDFOR
END
