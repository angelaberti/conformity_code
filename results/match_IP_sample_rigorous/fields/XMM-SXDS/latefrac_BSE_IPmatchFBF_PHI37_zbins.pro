...
PRO latefrac_BSE_IPmatchFBF_PHI37_zbins, outputFormat;
	field = 'xmm'
	dz_coeff = 2.0
	zmin = 0.2
	zmax = 1.0
	...
	zArray = [0.2, MEDIAN(dataIPallz.zprimus), 1.0]
...
;	dataIP  = dataIP[where(dataIP.targ_weight GE 1.)]
  ...
  FOR i=0,1 DO BEGIN
	...
	zsuffix = 'H' + STRTRIM(i,2)
	zlabel  = strtrim(string(z_low,format='(f20.2)'),1) + ' < z < ' + strtrim(string(z_high,format='(f20.2)'),1)

	IF (string(outputFormat) EQ 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/latefracplot_BSE_IPmatchFBF_PHI3.7_XMM-SXDS_' + zsuffix, THICK=5, /ENCAP
...
;	dataAll = dataAllallz
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/fields/XMM-SXDS/latefrac_' + zsuffix + '_targ_weight_IPmatchFBF_PHI3.7' + string_dR + '_BSE.fits', /create
  ENDFOR
END
