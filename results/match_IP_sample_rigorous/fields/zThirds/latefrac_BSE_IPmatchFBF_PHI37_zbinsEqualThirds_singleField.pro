...
; zbinsEqualThirds: late-type fractions are computed for three redshift bins with equal #s of IP

PRO latefrac_BSE_IPmatchFBF_PHI37_zbinsEqualThirds_singleField, outputFormat, field;, zmin, zmax;, dz_coeff, printEvery
	dz_coeff = 2.0
	zmin = 0.2
	zmax = 1.0
...
;	dataIP  = dataIP[where(dataIP.targ_weight GE 1.)]
  ...
  FOR i=0,2 DO BEGIN
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/latefracplot_BSE_IPmatchFBF_PHI3.7_z' + zsuffix, THICK=3, /ENCAP
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/fields/latefrac_' + zsuffix + '_targ_weight_IPmatchFBF_PHI3.7' + string_dR + '_BSE.fits', /create
  ENDFOR
END
