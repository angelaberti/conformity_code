...
PRO latefrac_BSE_allz_halo12_byField
	PHI = 3.7
...
;	zlabel  = strtrim(string(z_low,format='(f20.2)'),1) + ' < z < ' + strtrim(string(z_high,format='(f20.2)'),1)
  ...
  FOREACH f,fields DO BEGIN
	IF (f EQ 'cdfs      ') THEN (stringField = 'CDFS')
	IF (f EQ 'cfhtls_xmm') THEN (stringField = 'CFHTLS-XMM')
	IF (f EQ 'cosmos    ') THEN (stringField = 'COSMOS')
