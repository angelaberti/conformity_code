...
PRO latefrac_BSE_IPmatchFBF_PHI37_massThirdsEqualRange, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	dz_coeff = 2.0
	zmin = 0.2
	zmax = 1.0
  ...
  FOR i=0,2 DO BEGIN
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/latefracplot_BSE_IPmatchFBF_PHI37_' + massSuffix, THICK=3, /ENCAP
...
;	dataAll = dataAllallz[where( (dataAllallz.mstar GE mass_low) AND (dataAllallz.mstar LE mass_high) )]
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/massThirds/latefrac_' + massSuffix + '_targ_weight_IPmatchFBF_PHI3.7' + string_dR + '_BSE.fits', /create
  ENDFOR
END
