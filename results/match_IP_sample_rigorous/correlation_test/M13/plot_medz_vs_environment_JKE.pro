PRO plot_medz_vs_environment_JKE, outputFormat;, ptsPerBin, dR
	dN = 2
	ymin = 0.2
	ymax = 1
	yr = ymax-ymin
  ...
  IF (STRING(outputFormat) EQ 'ps') THEN BEGIN
	PS_OPEN, '~/latex/figures/medz_vs_environ_M13massLim', /ENCAP, THICK=5
...
;	  IF (Ndata NE !NULL) THEN z_Point = TOTAL((Ndata.IP_weight)*(Ndata.zIP))/TOTAL(Ndata.IP_weight) ELSE z_Point=0.
	  IF (Ndata NE !NULL) THEN z_Point = MEDIAN(Ndata.zIP) ELSE z_Point=0.
	  z_Array = [z_Array, z_Point]
	ENDFOR
	z_RegionArray = [[z_RegionArray], [z_Array]]
  ENDFOR
...
;	PRINT, Narray[0:-2]+0.5*dN
	...
	errorArray = []
	FOR i=0,n_elements(Narray)-2 DO BEGIN
;		PRINT, TRANSPOSE(z_RegionArray[i,*])
		errorVec = z_RegionArray[i,*]
...
;	    z_PointAllRegions = TOTAL((Ndata.IP_weight)*(Ndata.zIP))/TOTAL(Ndata.IP_weight)
	    z_PointAllRegions = MEDIAN(Ndata.zIP)
	  ...
	  ENDELSE
	  z_ArrayAllRegions = [z_ArrayAllRegions, z_PointAllRegions]
...
;	PLOT, [0,MAX(neighData.n_neighbors)], [0,0], LINESTYLE=1, xrange=[1,MAX(neighData.n_neighbors)+1], yrange=[ymin,ymax], /XLOG, $
	...
	PLOT, [0,MAX(neighData.n_neighbors)], [0,0], LINESTYLE=1, xrange=[0.9,100], yrange=[ymin,ymax], /XLOG, $
	  xtitle = textoidl('Neighbors [0.3 < R_{proj} < 4 Mpc]'), ytitle=textoidl('Median Redshift')
...
;	LEGEND, [textoidl('log(M_{*IP}/M_{\odot})'),'11.0-11.5', '10.5-11.0', '10.0-10.5', '9.5-10.0'], $
	LEGEND, [textoidl('log(M_{*IP}/M_{\odot} ) = 10.7-11.0'), '10.4-10.7', '10.1-10.4'], $
		PSYM=PSYMS, NUMBER=2, PSPACING=2, COLOR=cgColor(colors), BOX=0, /TOP, /RIGHT, CHARSIZE=1.15

	XYOUTS, 80, ymin+0.05*yr, textoidl('log(M_{*IP}/M_{\odot} ) - log(M_{*neigh}/M_{\odot} ) < 0.5'), ALIGNMENT=1.0, CHARSIZE=1.15
	XYOUTS, 80, ymin+0.125*yr, 'IP galaxies only', CHARSIZE=1.15, ALIGNMENT=1.0
      ENDIF

;	OPLOT, Narray[1:-1], z_ArrayAllRegions, PSYM=4
;	ERRPLOT, Narray[1:-1], z_ArrayAllRegions+errorArray, z_ArrayAllRegions-errorArray
	OPLOT, meanNweightArray, z_ArrayAllRegions, PSYM=psyms[m], COLOR=cgColor(colors[m])
	ERRPLOT, meanNweightArray, z_ArrayAllRegions+errorArray, z_ArrayAllRegions-errorArray, COLOR=cgColor(colors[m])
;	OPLOT, medNweightArray, z_ArrayAllRegions, PSYM=4;, COLOR=cgColor('green')
;	ERRPLOT, medNweightArray, z_ArrayAllRegions+errorArray, z_ArrayAllRegions-errorArray
...
;	PRINT, TRANSPOSE([[N_IParray], [Narray[1:-1]], [meanNweightArray], [medNweightArray]])
	PRINT, TRANSPOSE([[N_IParray], [Narray[1:-1]], [meanNweightArray], [z_ArrayAllRegions], [errorArray]])
