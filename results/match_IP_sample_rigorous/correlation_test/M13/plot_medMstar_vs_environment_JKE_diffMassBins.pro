PRO plot_medMstar_vs_environment_JKE_diffMassBins, outputFormat;, ptsPerBin, dR
	dN = 2
	ymin = 10
	ymax = 11
	yr = ymax-ymin
  ...
  IF (STRING(outputFormat) EQ 'ps') THEN BEGIN
	PS_OPEN, '~/latex/figures/medMstar_vs_environ_M13massLim_diffMassBins', /ENCAP, THICK=5
...
;	Narray = [0,10,10^1.5,100]
  ...
  ENDFOR
	...
	PLOT, meanNweightArray, mstar_ArrayAllRegions, PSYM=psyms[m], SYMSIZE=2, xtickformat=xtickformat, xtitle=xtitle, $
...
;	  xtitle = textoidl('Neighbors {0.3 < R_{proj} < 4 Mpc}'), ytitle=textoidl('Late-type Fraction of IPs')
	ERRPLOT, meanNweightArray, mstar_ArrayAllRegions+errorArray, mstar_ArrayAllRegions-errorArray

	PRINT, mstar_ArrayAllRegions, errorArray
	PRINT, ABS(mstar_ArrayAllRegions[0] - mstar_ArrayAllRegions[-1])
	PRINT, ABS(mstar_ArrayAllRegions[0] - mstar_ArrayAllRegions[-1])/SQRT(errorArray[0]^2+errorArray[-1]^2)	
