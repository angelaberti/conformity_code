PRO plot_completeness
	...
	dataAll = MRDFITS('~/conformity/results/default_parameters/allAboveMassCompLim-0.5.fits',1)
	dataAll = dataAll[WHERE(dataAll.targ_weight GE 1.)]

	dataIP  = MRDFITS('~/conformity/results/default_parameters/IP_data/zerodSFQ_IP_dz2.0_dm0.5.fits',1)
	dataIP  = dataIP[WHERE((dataIP.IP EQ 1) AND (dataIP.targ_weight GE 1.))]

	dataAllcons = MRDFITS('~/conformity/results/conservative_mass_cutoff/allAboveMassCompLim-0.0.fits',1)
	dataAllcons = dataAllcons[WHERE(dataAllcons.targ_weight GE 1.)]

	dataIPcons  = MRDFITS('~/conformity/results/match_IP_sample_rigorous/correlation_test/allAboveMassCompLim+0.5_IP.fits',1)
	dataIPcons  = dataIPcons[WHERE((dataIPcons.IP EQ 1) AND (dataIPcons.targ_weight GE 1.))]

	dataIP_M13  = MRDFITS('~/conformity/results/conservative_mass_cutoff/IP_data/zerodSFQ_IP_dz2.0_dm0.0.fits',1)
