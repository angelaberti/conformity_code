...
;	PS_OPEN, '~/latex/figures/medMstar_vs_environ_10.0_10.5', /ENCAP, THICK=5
	PS_OPEN, '~/latex/figures/medMstar_vs_environ_10.5_11.0', /ENCAP, THICK=5
...
;	PLOT, [0,MAX(neighData.n_neighbors)], [0,0], LINESTYLE=1, xrange=[1,MAX(neighData.n_neighbors)+1], yrange=[ymin,ymax], /XLOG, $
	...
	PLOT, [0,MAX(neighData.n_neighbors)], [0,0], LINESTYLE=1, xrange=[0.9,100], yrange=[ymin,ymax], /XLOG, $
	  xtitle = textoidl('Neighbors [0.3 < R_{proj} < 4 Mpc]'), ytitle=textoidl('Median Stellar Mass (M_{\odot})')
