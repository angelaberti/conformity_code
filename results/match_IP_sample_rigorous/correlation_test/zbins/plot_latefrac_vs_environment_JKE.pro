PRO plot_latefrac_vs_environment_JKE, outputFormat, m_input;, zmin, zmax;, ptsPerBin, dR
	dN = 2
	ymin = 0.3
	ymax = 1.1
	yr = ymax-ymin
...
;	massRanges = ['10.5_11.0']
;	massRanges = ['11.0_11.5']
...
;	zRanges = [0.2,MEDIAN(lowHalf.zIP),MEDIAN(zz.zIP),MEDIAN(highHalf.zIP),1.0]
	zRanges = [0.2, zzSorted[tt].zIP, zzSorted[2*tt].zIP, 1.0]	
	zRanges = [0.2,0.3,0.4,0.6,0.8,1.0]

 	colors = ['cyan', 'purple', 'orange', 'red', 'blue']
	PSYMS = [-1,-4,-5,-6,-7]
  ...
  IF (STRING(outputFormat) EQ 'ps') THEN BEGIN
	PS_OPEN, '~/latex/figures/latefrac_vs_environ_'+massRanges[m_input]+'_5zbins_complete', /ENCAP, THICK=5
...
;	LEGEND, [textoidl('IP z range'),'0.2-0.4','0.4-0.6','0.6-0.8','0.8-1.0'], $
	...
	IF (massRanges[m_input] EQ '10.0_10.5') THEN $
	  XYOUTS, 80, ymin+0.05*yr, textoidl('10.0 < log(M_{*IP}/M_{\odot}) < 10.5'), ALIGNMENT=1.0
	IF (massRanges[m_input] EQ '10.5_11.0') THEN $
