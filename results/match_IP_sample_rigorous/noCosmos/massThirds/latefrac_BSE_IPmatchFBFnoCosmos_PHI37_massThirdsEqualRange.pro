...
; massThirdsEqualRange: mass thirds with equal mass range in each third

PRO latefrac_BSE_IPmatchFBFnoCosmos_PHI37_massThirdsEqualRange, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	dz_coeff = 2.0
	zmin = 0.2
	zmax = 1.0
	...
	dataAllallz = MRDFITS('~/conformity/results/zerodSFQ_all_cart.fits', 1)
        ...
        dataAllallz = dataAllallz[where(dataAllallz.field NE 'cosmos    ')]
...
;	dataIP_M1 = dataIPallz[where(dataIPallz.mstar LE lowerMass)]
;	dataIP_M2 = dataIPallz[where( (dataIPallz.mstar GE lowerMass) AND (dataIPallz.mstar LE upperMass))]
;	dataIP_M3 = dataIPallz[where(dataIPallz.mstar GE upperMass)]
  ...
  FOR i=0,2 DO BEGIN
	...
	massSuffix = 'M' + strtrim(i+1,1) + 'equalRange'
	masslabel  = textoidl('M_{*} = [') + strtrim(string(mass_low,format='(f20.2)'),1) + ', ' + strtrim(string(mass_high,format='(f20.2)'),1) + ']'

	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/latefracplot_BSE_IPmatchFBFnoCosmos_PHI37_' + massSuffix, THICK=3, /ENCAP
...
;	dataAll = dataAllallz[where( (dataAllallz.mstar GE mass_low) AND (dataAllallz.mstar LE mass_high) )]
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/noCosmos/massThirds/latefrac_' + massSuffix + '_targ_weight_IPmatchFBFnoCosmos_PHI3.7' + string_dR + '_BSE.fits', /create
  ENDFOR
END
