...
PRO latefrac_errorCompare, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	!P.FONT=0
...
;	ymax = 0.89
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/latex/figures/unmatchedIPsampleCompare_pres', THICK=5, /ENCAP
		DEVICE, /INCH, XS=9, YS=5, SET_FONT='Palatino-Roman'
	ENDIF ELSE BEGIN
	        SET_PLOT, 'X'
		THICK=1
	ENDELSE
...
;	PLOT, Rplot_array, frac_IPSF, xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle='Projected Radius (Mpc)', ytitle='Late-type Fraction', POSITION=pos, $
	PLOT, Rplot_array, frac_IPSF, xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle=textoidl('R_{proj} (Mpc)'), ytitle=ytitle, ytickformat=ytickformat, POSITION=pos, $
	  /NODATA
