...
PRO latefrac_errorCompare_vertical, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	!P.FONT=0
...
;	ymax = 0.89
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/latex/figures/unmatchedIPsampleCompare_vertical', THICK=5, /ENCAP
		DEVICE, /INCH, XS=5, YS=8, SET_FONT='Palatino-Roman'
	ENDIF ELSE BEGIN
	        SET_PLOT, 'X'
		THICK=1
	ENDELSE
...
;	PLOT, Rplot_array, frac_IPSF, xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle='Projected Radius (Mpc)', ytitle='Late-type Fraction', POSITION=pos, $
	PLOT, Rplot_array, frac_IPSF, xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle=xtitle, xtickformat=xtickformat, ytitle=ytitle, ytickformat=ytickformat, POSITION=pos, $
	  /NODATA
;	  title = 'Field-by-Field Matched SF & Q IP Samples ' + textoidl('(\Deltaz=2.0)'), /NODATA
;	IF (i EQ 0) THEN XYOUTS, xmin+0.05*xr, ymin+0.90*yr, textoidl('Matched M_{*} Only'), ALIGNMENT=0.0, CHARSIZE=charsz
;	IF (i EQ 1) THEN XYOUTS, xmin+0.05*xr, ymin+0.90*yr, textoidl('Matched M_{*} & Redshift'), ALIGNMENT=0.0, CHARSIZE=charsz
;	IF (i GT 1) THEN
...
;		LEGEND, ['Late-type IP', 'Early-type IP'], LINESTYLE=[0,2], COLOR=[SFcolor, Qcolor], CHARSIZE=charsz, BOX=0, /TOP, /RIGHT
		LEGEND, ['', 'SF IP', 'Q IP'], LINESTYLE=[1,0,2], COLOR=[cgColor('white'), SFcolor, Qcolor], CHARSIZE=charsz, BOX=0, /TOP, /RIGHT
