...
PRO latefracplot_BSE_allz_halo12_wHist, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	!P.FONT=0
...
; eliminate data with targ_weight < 1
	...
	IF (i EQ 3) THEN datafile = '~/results/match_IP_sample_rigorous/unmatchedExample/halo12test/latefrac_allz_targ_weight_IPmatchFBF_PHI3.7_dR1Mpc_BSE_halo12.fits' ELSE $
	  datafile = '~/results/match_IP_sample_rigorous/unmatchedExample/halo12test/latefrac_' + tags[i] + '_allz_dR1Mpc_BSE.fits'
...
;	PRINT, POSITION_ARRAY[3*i]
	...
	PLOT, Rplot_array, frac_IPSF, xrange=[xmin,xmax], yrange=[ymin,ymax], $
		...
		POSITION=POSITION_ARRAY[3*i], /NORMAL, /NODATA
...
; 2nd PLOT: MASS HISTOGRAMS
	...
	bins = xmin + b*FINDGEN(CEIL(xr/b)+1)
	PRINT, bins

	PLOT, bins, HISTOGRAM(dataIP.mstar, bin=b), xrange=[xmin,xmax], yrange=[ymin,ymax], PSYM=10, $
...
;	PLOTHIST, dataIP.zprimus, bin=b, xrange=[xmin,xmax], yrange=[ymin,ymax], $
