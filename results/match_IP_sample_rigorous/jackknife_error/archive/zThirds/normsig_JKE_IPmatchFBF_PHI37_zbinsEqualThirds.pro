...
PRO normsig_JKE_IPmatchFBF_PHI37_zbinsEqualThirds, outputFormat
	PHI = 3.7
...
;	zlabel  = strtrim(string(z_low,format='(f20.2)'),1) + ' < z < ' + strtrim(string(z_high,format='(f20.2)'),1)
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/normsigplot_JKE_IPmatchFBF_PHI37_zbinsEqualThirds', THICK=3, /ENCAP
...
; ***** Q IP *****
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/jackknife_error/zThirds/normsig_' + zsuffix + '_targ_weight_IPmatchFBF_PHI' + stringPHI + string_dR + '_JKE.fits', /CREATE
  ENDFOR
END
