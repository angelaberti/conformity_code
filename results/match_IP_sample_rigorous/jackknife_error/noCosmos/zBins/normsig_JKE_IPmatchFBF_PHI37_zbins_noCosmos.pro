...
PRO normsig_JKE_IPmatchFBF_PHI37_zbins_noCosmos, n_bins
	PHI = 3.7
...
;	zsuffix = 'allz'
	...
	dataIP	= IPdataAllz[where((IPdataAllz.zprimus GE z_low) AND (IPdataAllz.zprimus LE z_high) AND (IPdataAllz.field NE 'cosmos    '))]
	dataAll	= allDataAllz[where((allDataAllz.zprimus GE z_low) AND (allDataAllz.zprimus LE z_high) AND (allDataAllz.field NE 'cosmos    '))]
...
;	  title = 'Field-by-Field Matched SF & Q IP Samples ' + textoidl('(\Deltaz=2.0)'), /NODATA
	...
	OPLOT, Rproj_array+0.5*dRproj, normsigArray
...
;	SET_PLOT, 'X'
	...
	outputStruct    = replicate(templateRow, n_elements(Rproj_array)-1)
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/jackknife_error/noCosmos/zBins/normsig_' + zSuffixArray[n] + '_targ_weight_IPmatchFBF_PHI' + stringPHI + string_dR + '_JKE_noCosmos.fits', /CREATE
  ENDFOR
END
