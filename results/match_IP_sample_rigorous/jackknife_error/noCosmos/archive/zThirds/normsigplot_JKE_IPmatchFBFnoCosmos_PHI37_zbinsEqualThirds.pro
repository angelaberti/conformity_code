...
PRO normsigplot_JKE_IPmatchFBFnoCosmos_PHI37_zbinsEqualThirds, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	!P.FONT=0
...
;	dataIPallz  = MRDFITS('~/conformity/results/match_IP_sample_rigorous/matchedIPsampleFBF_PHI3.7.fits', 1)
	...
	; divide IP population into 3 redshift bins with equal numbers of IPs
        ...
        PRINT, MINMAX(dataIPallz[where(dataIPallz.field NE 'cosmos    ')].mstar)
        PRINT, MINMAX(dataIPallz.mstar)

        dataIPSF = dataIPallz[where( (dataIPallz.SFQ EQ 1) AND (dataIPallz.field NE 'cosmos    ') )]
        dataIPQ  = dataIPallz[where( (dataIPallz.SFQ EQ 0) AND (dataIPallz.field NE 'cosmos    ') )]
...
;	colors = ['blue', 'green', 'red']
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/latex/figures/normsigplot_JKE_IPmatchFBFnoCosmos_PHI37_zbinsEqualThirds', THICK=5, /ENCAP
  ...
  FOR i=0,2 DO BEGIN
	...
	data = MRDFITS('~/conformity/results/match_IP_sample_rigorous/jackknife_error/noCosmos/zThirds/normsig_' + zsuffix + '_targ_weight_IPmatchFBFnoCosmos_PHI' + stringPHI + string_dR + '_JKE.fits', 1)
...
;		+ textoidl('; \sigma_{3-5}=') + getSigmaRange_JKE(data, 3, 4)]
  ...
  ENDFOR
	...
	LEGEND, legend, LINESTYLE=ls, COLOR=cgColor(colors), BOX=0, /TOP, /RIGHT, NUMBER=2, PSPACING=3
