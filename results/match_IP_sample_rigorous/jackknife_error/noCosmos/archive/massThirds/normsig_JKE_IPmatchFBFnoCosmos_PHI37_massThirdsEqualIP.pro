...
PRO normsig_JKE_IPmatchFBFnoCosmos_PHI37_massThirdsEqualIP, outputFormat
	PHI = 3.7
...
;	PRINT, orderedMasses
        ...
        massArray = [min(IPdataAllz.mstar), lowerMass, upperMass, max(IPdataAllz.mstar)]
...
;	xmax = n_annuli*dRproj
  ...
  FOR i=0,2 DO BEGIN
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/normsigplot_JKE_IPmatchFBFnoCosmos_PHI37_' + massSuffix, THICK=3, /ENCAP
...
; ***** Q IP *****
	...
	PLOT, Rproj_array[0:plotMax]+0.5*dRproj, normsigArray[0:plotMax], xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle='Projected Radius (Mpc)', ytitle='Normalized Conformity Signal', $
	  title = 'Field-by-Field Matched SF & Q IP Samples ' + textoidl('(\Deltaz=2.0)'), /NODATA

	OPLOT, Rproj_array[0:plotMax]+0.5*dRproj, normsigArray[0:plotMax];, LINESTYLE=0
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/jackknife_error/noCosmos/massThirds/normsig_' + massSuffix + '_targ_weight_IPmatchFBFnoCosmos_PHI' + stringPHI + string_dR + '_JKE.fits', /CREATE
  ENDFOR
END
