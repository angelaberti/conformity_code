...
PRO normsigplot_JKE_IPmatchFBFnoCosmos_PHI37_massThirdsEqualIP, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	PHI = 3.7
...
;	PRINT, Rproj_array
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/normsigplot_JKE_IPmatchFBFnoCosmos_PHI37_massThirdsEqualIP', THICK=3, /ENCAP
...
;	dataAll = dataAllallz[where( (dataAllallz.mstar GE mass_low) AND (dataAllallz.mstar LE mass_high) )]
  ...
  ENDIF
	...
	OPLOT, Rproj_array[0:plotMax]+0.5*dRproj, normsig[0:plotMax], color=cgcolor(colors[i])
