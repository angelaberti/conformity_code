...
PRO matchSamplesFBF_SFRquarts, outputFormat
	targPhi = -3.7
...
; TAKE HIGHEST AND LOWEST SFR/M* QUARTILES, THEN MATCH Z & M* FIELD BY FIELD
...
; VALUES FROM BIMODAL DISTRIBUTION OF *ALL* GALAXIES
;	SFRperM_low	= -11.31
;	SFRperM_high	= -9.42 
; VALUES FROM BIMODAL DISTRIBUTION OF ALL IP CANDIDATES ABOVE M13 MASS LIMIT
	SFRperM_low	= -11.4746
	SFRperM_high	= -9.83967

	dataIP_highLow	= dd[WHERE( ((dd.SFR-dd.mstar) GE SFRperM_high ) OR ((dd.SFR-dd.mstar) LE SFRperM_low) )]
;	dataIP_low	= dd[WHERE( (dd.SFR-dd.mstar) LE SFRperM_low )]
...
;	targPhi = -3.5
  ...
  FOR f=0,n_elements(fields)-1 DO BEGIN
	data = dataIP_highLow[WHERE(dataIP_highLow.field EQ fields[f])]
;	PRINT, 'FIELD: ' + strcompress(fields[f])
;	PRINT, 'IPs: ' + strcompress(n_elements(data))
  
	IF (string(outputFormat) EQ 'ps') THEN BEGIN
;	        PS_OPEN, '~/figures/matchedIPsampleFBF/matchedIPsampleFBF_M13highMassCut_' + strcompress(strtrim(fields[f],2)), /ENCAP, THICK=3
...
;		PRINT, weights[where(weights GT 5)]
	...
	PRINT, n_elements(allIP)
	PRINT, n_elements(allIP[WHERE(allIP.SFQ EQ 1)]), n_elements(allIP[WHERE(allIP.SFQ EQ 0)])
	PRINT, MEDIAN(allIP[WHERE(allIP.SFQ EQ 1)].mstar), MEDIAN(allIP[WHERE(allIP.SFQ EQ 0)].mstar)
	PRINT, MEDIAN(allIP[WHERE(allIP.SFQ EQ 1)].zprimus), MEDIAN(allIP[WHERE(allIP.SFQ EQ 0)].zprimus)
