...
PRO normsigplot_JKE_IPmatchFBF_PHI37_zbins_halo12_noCosmos, n_bins, outputFormat
	!P.FONT=0
...
; eliminate data with targ_weight < 1
	...
	IF (string(outputFormat) EQ 'ps') THEN BEGIN
		PS_OPEN, '~/figures/matchedIPsampleFBF/normsigplot_JKE_IPmatchFBF_PHI37_' + figureSuffix + '_halo12_noCosmos', THICK=5, /ENCAP
  ...
  FOR n=0,n_elements(zArray)-2 DO BEGIN
	...
	dataAll = dataAllallz[where( (dataAllallz.zprimus GE z_low) AND (dataAllallz.zprimus LE z_high) AND (dataAllallz.field NE 'cosmos    ') )]
	...
	data = MRDFITS('~/conformity/results/match_IP_sample_rigorous/jackknife_error/halo12/noCosmos/zBins/normsig_' + zSuffixArray[n] + '_targ_weight_IPmatchFBF_PHI' + stringPHI + string_dR + '_JKE_halo12_noCosmos.fits', 1)
