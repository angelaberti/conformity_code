PRO matchedSamplePlot_allFields_noContours, outputFormat
	!P.FONT=0
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/latex/figures/matchedSamplePlot_allFields_noContours', /ENCAP, THICK=5
...
;	ytitle = textoidl('log (M_{stellar} / M_{\odot})')
	...
	; SET UP PLOT AREA
	PLOT, data.zprimus, data.mstar, xrange=[xmin, xmax], yrange=[yminPlot, ymaxPlot], xtitle=xtitle, ytitle=ytitle, COLOR=textColor, /NODATA
...
;	CONTOUR, MIN_CURVE_SURF(cont, XGRID=[0.2, z_binwidth], YGRID=[MIN(IPselect.mstar), mass_binwidth], GS=[z_binwidth,mass_binwidth]), $
;		xpts+0.25*z_binwidth, ypts+0.25*mass_binwidth, $
;		C_THICK=4, /OVERPLOT

;	FOR i=0,n_elements(xpts)-1 DO BEGIN
;	  FOR j=0,n_elements(ypts)-1 DO BEGIN
;	    IF (cont[i,j] GT 0) THEN XYOUTS, xpts[i]+0.5*z_binwidth, ypts[j]+0.33*mass_binwidth, STRTRIM(cont[i,j],2), ALIGNMENT=0.5, CHARSIZE=1.5
;	  ENDFOR
;	ENDFOR
