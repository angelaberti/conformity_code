PRO neighborHist_z_byField, binwidth, outputFormat
	PHI = 3.7
...
;	PRINT, Rproj_array
	...
	IF (string(outputFormat) EQ 'ps') THEN BEGIN
		PS_OPEN, '~/latex/figures/neighborHist_z_byField', THICK=5, /ENCAP
...
;	binwidth = 0.25
  ...
  FOR f=0,n_elements(fields)-1 DO BEGIN
	...
	h = HIST_POINTS(dataSF[WHERE(dataSF.field EQ fields[f])].zprimus, binwidth)
    IF (f EQ 0) THEN $
	PLOT, h[*,0], h[*,1], /NODATA, xrange=[xmin,xmax], yrange=[0,220], xtitle='Redshift', ytitle='Number', POSITION=posArray[*,2*f] $
    ELSE $
	PLOT, h[*,0], h[*,1], /NODATA, xrange=[xmin,xmax], yrange=[0,220], xtickformat='(A1)', ytitle='Number', POSITION=posArray[*,2*f]

;	OPLOT, h[*,0], h[*,1], COLOR=cgColor('blue'), LINESTYLE=2
;	h = HIST_POINTS(dataQ[WHERE(dataQ.field EQ fields[f])].zprimus, binwidth)
;	OPLOT, h[*,0], h[*,1], PSYM=0, COLOR=cgColor('red'), LINESTYLE=2
	h = HIST_POINTS(dataIPSF[WHERE(dataIPSF.field EQ fields[f])].zprimus, binwidth)
	OPLOT, h[*,0], h[*,1], PSYM=0, COLOR=cgColor('blue')
	h = HIST_POINTS(dataIPQ[WHERE(dataIPQ.field EQ fields[f])].zprimus, binwidth)
...
;		NUMBER=2, PSPACING=3, CHARSIZE=0.8*charsz
	LEGEND, ['SF IP', 'Q IP'], COLOR=cgColor(['blue','red']), LINESTYLE=[0,0], /TOP, /LEFT, BOX=0, $
		NUMBER=2, PSPACING=3, CHARSIZE=0.8*charsz
  
;	get_hist_data, dataIPfield, dataAllfield, dz_coeff, 1, neighborRedshiftsIPSF
;	SAVE, neighborRedshiftsIPSF, FILENAME='neighborRedshiftsIPSF_'+stringField+'.sav'
	RESTORE, 'neighborRedshiftsIPSF_'+stringField+'.sav'
	histPointsIPSF	= HIST_POINTS(neighborRedshiftsIPSF, binwidth)
;	histPointsIPSF	= HIST_POINTS(neighborRedshiftsIPSF[UNIQ(neighborRedshiftsIPSF,SORT(neighborRedshiftsIPSF))], binwidth)
	x_IPSF = histPointsIPSF[*,0]
	y_IPSF = histPointsIPSF[*,1]

;	get_hist_data, dataIPfield, dataAllfield, dz_coeff, 0, neighborRedshiftsIPQ
;	SAVE, neighborRedshiftsIPQ, FILENAME='neighborRedshiftsIPQ_'+stringField+'.sav'
	RESTORE, 'neighborRedshiftsIPQ_'+stringField+'.sav'
	histPointsIPQ	= HIST_POINTS(neighborRedshiftsIPQ, binwidth)
;	histPointsIPQ	= HIST_POINTS(neighborRedshiftsIPQ[UNIQ(neighborRedshiftsIPQ,SORT(neighborRedshiftsIPQ))], binwidth)
...
;	PLOT, FINDGEN(10), xrange=[MIN([MIN(x_IPSF),MIN(x_IPQ)])-binwidth/2., MAX([MAX(x_IPSF),MAX(x_IPQ)])+binwidth/2.], $
	...
	AXIS, YAXIS=1
    IF (f EQ 4) THEN $
	LEGEND, ['neigh of SF IP', 'neigh of Q IP'], COLOR=cgColor(['blue','red']), LINESTYLE=[0,0], /TOP, /LEFT, BOX=0, $
		NUMBER=2, PSPACING=3, CHARSIZE=0.8*charsz
...
;	OPLOT, histPointsIPQ[*,0], histPointsIPQ[*,1], LINESTYLE=f, COLOR=cgColor('RED'+STRING(f+3))
	...
	XYOUTS, xmin+0.95*xr, ymin+0.80*800, stringFieldOut[f], ALIGNMENT=1.0, CHARSIZE=charsz
...
PRO get_hist_data, dataIP, dataAll, dz_coeff, IPSFstatus, neighborRedshifts
	data_isIP = dataIP
	data = dataAll
...
;	FOR i=0,199 DO BEGIN
		...
		IF (IPneighbors NE !NULL) THEN (neighborRedshifts = [neighborRedshifts, IPneighbors.zprimus])
