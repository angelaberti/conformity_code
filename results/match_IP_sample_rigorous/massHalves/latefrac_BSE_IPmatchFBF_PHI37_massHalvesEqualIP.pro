...
; massHalvesEqualIP: mass halves with equal # of IP in each third

PRO latefrac_BSE_IPmatchFBF_PHI37_massHalvesEqualIP, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	dz_coeff = 2.0
	zmin = 0.2
	zmax = 1.0
...
;	massArray = [min(dataIPallz.mstar), lowerMass, upperMass, max(dataIPallz.mstar)]
	massArray = [min(dataIPallz.mstar), median(dataIPallz.mstar), max(dataIPallz.mstar)]
  ...
  FOR i=0,1 DO BEGIN
	...
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchedIPsampleFBF/latefracplot_BSE_IPmatchFBF_PHI3.7_' + massSuffix, THICK=3, /ENCAP
...
;	dataAll = dataAllallz[where( (dataAllallz.mstar GE mass_low) AND (dataAllallz.mstar LE mass_high) )]
	...
	MWRFITS, outputStruct, '~/conformity/results/match_IP_sample_rigorous/massHalves/latefrac_' + massSuffix + '_targ_weight_IPmatchFBF_PHI3.7' + string_dR + '_BSE.fits', /create
  ENDFOR
END
