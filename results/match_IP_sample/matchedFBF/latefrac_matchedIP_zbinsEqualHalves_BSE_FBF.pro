...
PRO latefrac_matchedIP_zbinsEqualHalves_BSE_FBF, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	dz_coeff = 2.0
	zmin = 0.2
	zmax = 1.0

        dataIPallz  = MRDFITS('~/conformity/results/match_IP_sample/matchedIPsample_FBF.fits', 1)
	...
	zlow_medianSFmass  = median(dataLower[where(dataLower.SFQ EQ 1)].mstar)
	zlow_medianQmass   = median(dataLower[where(dataLower.SFQ EQ 0)].mstar)
	zhigh_medianSFmass = median(dataUpper[where(dataUpper.SFQ EQ 1)].mstar)
	zhigh_medianQmass  = median(dataUpper[where(dataUpper.SFQ EQ 0)].mstar)

	zArray = [0.2, med_z, 1.0]
	PRINT, med_z
...
;		PRINT, zlabel
        	...
        	IF (string(outputFormat) eq 'ps') THEN BEGIN
        	        PS_OPEN, '~/figures/latefracplot_matchedIP_z' + zsuffix + '_BSE_FBF', THICK=3, /ENCAP
		...
		XYOUTS, xmin+0.95*xr, ymin+0.20*yr, 'SF IP: ' + bigint(n_elements(dataIP[where(dataIP.SFQ EQ 1)])) + ', Median M* ' + strtrim(string(median(dataIP[where(dataIP.SFQ EQ 1)].mstar), format='(f20.2)'),1), ALIGNMENT=1.0
		XYOUTS, xmin+0.95*xr, ymin+0.15*yr, 'Q IP: ' + bigint(n_elements(dataIP[where(dataIP.SFQ EQ 0)])) + ', Median M* ' + strtrim(string(median(dataIP[where(dataIP.SFQ EQ 0)].mstar), format='(f20.2)'),1), ALIGNMENT=1.0
		...
		MWRFITS, outputStruct, '~/conformity/results/match_IP_sample/latefrac_' + zsuffix + '_targ_weight_matchedIPsample_BSE_FBF.fits', /create
	ENDFOR
END
