...
PRO latefracplot_matchedIP_zbinsEqualHalves_samePlot, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/latefracplot_matchedIP_zbinsEqualHalves', THICK=3, /ENCAP
...
;		dataAll = dataAllallz[where( (dataAllallz.zprimus GE z_low) AND (dataAllallz.zprimus LE z_high) )]
		...
		IF i EQ 0 THEN BEGIN
			...
			LEGEND, legend, LINESTYLE=[0,0], color=cgColor(colors), BOX=0, /BOTTOM, /LEFT
		ENDIF
