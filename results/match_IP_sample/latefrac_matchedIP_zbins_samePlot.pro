...
PRO latefrac_matchedIP_zbins, outputFormat;, zmin, zmax;, dz_coeff, printEvery
	dz_coeff = 2.0
	zmin = 0.2
	zmax = 1.0
	...
	FOR i=0,3 DO BEGIN
		...
		IF (string(outputFormat) eq 'ps') THEN BEGIN
		        PS_OPEN, '~/figures/latefracplot_matchedIP_z' + zsuffix, THICK=3, /ENCAP
		...
		XYOUTS, xmin+0.95*xr, ymin+0.90*yr, 'Weighted by TARG_WEIGHT', ALIGNMENT=1.0
	        ...
	        FOR j=0,n_annuli-1 DO BEGIN
	                newRow  = create_struct('Rmin', Rproj_array[j], 'Rmax', Rproj_array[j+1], $
	                  'n_tot_IPSF', n_tot_IPSF[j], 'n_late_IPSF', n_late_IPSF[j], 'n_tot_IPQ', n_tot_IPQ[j], 'n_late_IPQ', n_late_IPQ[j])
	                print, newRow
