PRO norm_diff_plot_matchedIP_zbinsEqualHalves_FBF, outputFormat
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/norm_diff_plot_matchedIP_zbinsEqualHalves_FBF', THICK=3, /ENCAP
	...
	colors = [cgColor('Black'), cgColor('Magenta'), cgColor('Blue')]
...
;	zArray = [0.2, 0.502986, 0.699939, 1.0]
	zArray = [0.2, 0.606093, 1.0]

	FOR i=0,1 DO legend = [legend, 'z = [' + strtrim(string(zArray[i], format='(f20.2)'),1) + ', ' + strtrim(string(zArray[i+1], format='(f20.2)'),1) + ']']
	...
	;	ELSE zrange = strtrim(string(0.2 + 0.2*(i-1), format='(f20.1)'),1) + '_' + strtrim(string(0.2 + 0.2*(i), format='(f20.1)'),1)
		ELSE zrange = 'H' + strtrim(i,1)
		data = MRDFITS('~/conformity/results/match_IP_sample/latefrac_' + zrange + '_targ_weight_matchedIPsample_BSE_FBF.fits', 1)
...
;		PRINT, Rmin, Rmax
	...
	LEGEND, legend, LINESTYLE=[2,0,0], COLOR=colors, BOX=0, /BOTTOM, /RIGHT
