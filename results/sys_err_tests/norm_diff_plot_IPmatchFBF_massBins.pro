PRO norm_diff_plot_IPmatchFBF_massBins, outputFormat
	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/matchIPsamplesFBF/normDiffPlot_IPmatchFBF_massBins', THICK=3, /ENCAP
...
;	colors = [cgColor('Yellow'), cgColor('Magenta'), cgColor('Blue'), cgColor('Cyan'), cgColor('Green')]
	...
	massArray = [9.16133, 10.6748, 10.9637, 11.4058]

        dataIPallm  = MRDFITS('~/conformity/results/match_IP_sample_rigorous/matchedIPsampleFBF_M13highMassCut.fits', 1)
...
;		legend = [legend, 'M* = [' + decimal(massArray[i],2) + ', ' + decimal(massArray[i+1],2) + ']']
		...
		; latefrac_M2_targ_weight_IPmatchFBF_M13highMassCut_dR1Mpc_BSE.fits
		data = MRDFITS('~/conformity/results/match_IP_sample_rigorous/massThirds/latefrac_' + mrange + '_targ_weight_IPmatchFBF_M13highMassCut_dR1Mpc_BSE.fits', 1)
		...
		OPLOT, Rmin+0.5*(Rmax[1]-Rmax[0]), norm_diff, LINESTYLE=0, COLOR=colors[i]
;		PRINT, Rmin, Rmax
	ENDFOR
