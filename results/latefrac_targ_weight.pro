...
PRO latefrac_targ_weight, input_dm, outputFormat;, zmin, zmax;, dz_coeff, printEvery

        string_dm = strtrim(strcompress(string(input_dm, format='(f20.1)')),1)

	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/latefracplot_targ_weight_' + string_dm + 'dex_zall', THICK=5, /ENCAP
