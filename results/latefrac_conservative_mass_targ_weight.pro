...
PRO latefrac_conservative_mass_targ_weight, outputFormat;, zmin, zmax;, dz_coeff, printEvery
;       string_dm = strtrim(strcompress(string(input_dm, format='(f20.1)')),1)

	IF (string(outputFormat) eq 'ps') THEN BEGIN
	        PS_OPEN, '~/figures/latefracplot_conservative_mass_targ_weight', THICK=5, /ENCAP
...
;	PRINT, 'Input data: ', zerodInputFile
	...
	PLOT, Rproj_array, float(neigh_late_grand_total)/neigh_all_grand_total, xrange=[xmin,xmax], yrange=[ymin,ymax], xtitle='Projected Radius (Mpc)', ytitle='Late-type Fraction', $
	  title = 'Moustakas13 Mass Limit ' + textoidl('(\Deltaz=2.0)'), /NODATA
	...
	SET_PLOT, 'X'
        ...
        mwrfits, outputStruct, '~/results/conservative_mass_cutoff/latefrac_data_hist/latefrac_' + strtrim(strcompress(string(zmin, format='(f20.1)')), 1) + '_' $
