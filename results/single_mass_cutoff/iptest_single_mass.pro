...
PRO iptest_single_mass, dz_coeff ; , zerodInputFile, dm, dz_coeff, printEvery ; finds isolated primaries in a given field
	path = '../'
...
; make initial mass cut based on field, z, and star-forming status to get sample to test for IP status
	IPcands    = zerodInput[where(zerodInput.mstar ge 10.)]
	nonIPcands = zerodInput[where(zerodInput.mstar le 10.)]
;	IPcands    = mrdfits('../allAboveMassCompLim-' + strtrim(strcompress(string(dm, format='(f20.1)')), 1) + '.fits', 1)
;	nonIPcands = mrdfits('../allBelowMassCompLim-' + strtrim(strcompress(string(dm, format='(f20.1)')), 1) + '.fits', 1)
...
;	IF outputIndex ne n_elements(zerodInput_zcut) THEN message, "outputIndex mismatch"
	...
	mwrfits, zerodOutput, 'IP_data/zerodSFQ_IP_dz' + strtrim(strcompress(string(dz_coeff, format='(f20.1)')),1) + '_singleMass.fits', /create
END
