PRO zhist_IPstatus, outputFormat
        IF (string(outputFormat) eq 'ps') THEN BEGIN
		SET_PLOT, 'ps'
		DEVICE, file="../figures/zhist_IPstatus.ps", /LANDSCAPE
        ENDIF ELSE BEGIN
                SET_PLOT, 'X'
        ENDELSE
...
;	!y.margin=[3.5,1] ; bottom, top
	...
	data = mrdfits(path+'zerodSFQ_IP_dz1.0_dm0.5.fits', 1)
	
	dataIP  = data[where(data.IP eq 1)]
	dataNIP = data[where(data.IP eq 0)]

	plothist, data.zprimus,  bin=binsize, linestyle=0, thick=linethickness, yrange=[0,2200], xtitle="redshift", title="Conservative Mass Cutoff"
	plothist, dataIP.zprimus,  bin=binsize, linestyle=2, thick=linethickness, /overplot
	plothist, dataNIP.zprimus, bin=binsize, linestyle=1, thick=linethickness, /overplot

	LEGEND, ["all", "IP", "non-IP"], linestyle=[0,2,1], box=0, charsize=1
